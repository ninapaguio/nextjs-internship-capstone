...
function SheetOverlay({
	...
	return (
		<ModalOverlayPrimitive
			...
			isDismissable
			className={cn(
				"fixed inset-0 z-70 bg-black/30 transition-opacity duration-150 data-entering:opacity-0 data-exiting:opacity-0 supports-backdrop-filter:backdrop-blur-sm",
				className,
			)}
			{...props}
		>
				...
				>
					...
					{showCloseButton && (
						<TooltipTrigger delay={400}>
							...
							<Tooltip placement="left">Close panel</Tooltip>
						</TooltipTrigger>
					)}
				</SheetPrimitive>
			</ModalPrimitive>
...
function SheetFooter({ className, ...props }: React.ComponentProps<"div">) {
	return (
		<div
			...
			className={cn(
				"mt-auto flex flex-col gap-2 border-t bg-background p-6",
				className,
			)}
			{...props}
		/>
